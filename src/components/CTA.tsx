...
const CTA = () => {
  ...
  return (
    ...
    <section className="luz-chega relative isolate bg-surface px-8 pb-24">
      <div className="filete mx-auto grid max-w-7xl justify-items-start gap-4 border-t border-white/10 pt-16">
        <h2 className="revela font-display text-xl font-medium lowercase tracking-tight text-white" style={sequencia(0)}>{t('cta.title')}</h2>
        <p className="revela max-w-[60ch] text-[15px] leading-relaxed text-on-surface-variant" style={sequencia(1)}>
          <ComMarcas>{t('cta.subtitle')}</ComMarcas>
        </p>
        <div className="revela flex flex-wrap items-center gap-x-5 gap-y-2" style={sequencia(2)}>
