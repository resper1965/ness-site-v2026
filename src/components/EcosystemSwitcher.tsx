...
const MARCAS: { marca: Brand; nome: string; descricao: string }[] = [
  ...
  { marca: 'forense', nome: 'forense.io', descricao: 'perícia digital e investigação' },
];
  ...
  return (
    <div ref={caixa} className="relative">
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 border border-white/10 hover:border-primary-container/40 transition-colors focus-visible:ring-2 focus-visible:ring-primary-container"
      >
        ...
        <div id="ecossistema" className="absolute left-0 top-full pt-3 w-[260px] z-50">
          <div className="bg-surface-container-low/98 backdrop-blur-xl rounded-2xl border border-white/10 p-2 nebula-shadow">
            {MARCAS.map((m) => (
              ...
              >
                <span className="block text-xs font-bold text-white lowercase-all">
                  {m.nome}{m.marca !== 'forense' && <BlueDot />}
                </span>
