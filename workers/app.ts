 ...
 *
 * `static.cloudflareinsights.com` e `cloudflareinsights.com` não são nossos:
 * é o beacon que a Cloudflare injeta sozinha no HTML quando o Web Analytics
 * está em setup automático. Sem estas duas entradas, a nossa própria CSP
 * bloquearia a medição de campo — e ninguém saberia por quê.
 */
const csp = (nonce: string) =>
  `default-src 'self'; script-src 'self' 'nonce-${nonce}' https://challenges.cloudflare.com https://static.cloudflareinsights.com; style-src 'self' 'unsafe-inline'; font-src 'self'; img-src 'self' data: blob: https://*.ness.com.br https://*.r2.dev https://images.unsplash.com https://ui-avatars.com; connect-src 'self' https://canal.ness.com.br https://cloudflareinsights.com https://*.cloudflare.com https://*.ingest.us.sentry.io; frame-src https://challenges.cloudflare.com; frame-ancestors 'none'; object-src 'none'; base-uri 'self'; form-action 'self'; upgrade-insecure-requests`;
